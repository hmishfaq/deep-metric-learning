...
# attributes      classes.txt     images          parts           test_class_label.txt    train_class_label.txt   train_test_split.txt
    ...
    #     CUB_t('../datasets', train=True, download=True,
...
class CUB_t(data.Dataset):
    ...
    def __init__(self, root,  n_train_triplets=50000, n_test_triplets=10000, train=True, transform=None, target_transform=None, download=False):
        ...
        if self.train:
            colnames = ['idx','labels']
            df = pd.read_csv('train_class_label.txt',names=colnames, sep = " ")
            self.train_data = df['idx'].tolist()
    ...
    def make_triplet_list(self, ntriplets):
        ...
        if self.train:
            np_labels = np.array(self.train_labels) #self.train_labels.numpy()
            filename = self.train_triplet_file
        else:
            np_labels = np.array(self.test_labels) #self.test_labels.numpy()
            filename = self.test_triplet_file
        triplets = []
        for class_idx in range(1,3): #later 3 should be 200+1 = num_classes+1
            ...
            for i in range(a.shape[0]):
                ...
                triplets.append([self.train_data[anchor],self.train_data[positive],self.train_data[negative]])          
